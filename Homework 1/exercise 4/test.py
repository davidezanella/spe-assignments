...
start, end = st.t.interval(0.95, len(values)-1, loc=np.mean(values), scale=st.sem(values))
print("Confidence interval for mean with gamma 95%:")
print("\t[", start, ";", end, "]")

start, end = st.t.interval(0.99, len(values)-1, loc=np.mean(values), scale=st.sem(values))
...
for i in values:
    x *= i
print("Geometric mean sample values:", x ** (1 / (len(values))))
